...
const Gallery = () => {
  ...
  const images = [
    ...
    { src: '/gallery/expert-pipework.jpg', title: 'Expert Copper Pipework' },
    { src: '/gallery/pipework-1.jpg', title: 'Precision Manifold Pipework' },
    { src: '/gallery/company-van.jpg', title: 'Our Fully Equipped Fleet' },
    ...
    { src: '/gallery/radiator-wood-wall.jpg', title: 'Bespoke Radiator Fitting' },
    ...
    <section id="gallery" className="section" style={{ background: 'var(--bg-color-alt)', position: 'relative', zIndex: 10 }}>
      <div className="container">
        ...
        <div style={{ position: 'relative', maxWidth: '1000px', margin: '0 auto' }}>
          ...
          >
            {images.map((item, i) => (
              <div key={i} style={{ minWidth: '100%', scrollSnapAlign: 'center', borderRadius: '16px', overflow: 'hidden', position: 'relative', height: '500px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#000' }}>
                <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${item.src})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(20px) brightness(0.5)', transform: 'scale(1.1)', zIndex: 0 }} />
                <img src={item.src} alt={item.title} loading="lazy" style={{ position: 'relative', width: '100%', height: '100%', objectFit: 'contain', zIndex: 1, filter: 'contrast(1.15) brightness(1.05) saturate(1.1)' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px 24px', background: 'linear-gradient(to top, rgba(0,0,0,0.9) 0%, transparent 100%)', zIndex: 2 }}>
                  <h3 style={{ color: 'white', margin: 0, fontSize: '1.5rem', fontWeight: 600, textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>{item.title}</h3>
                </div>
              </div>
            ))}
          </div>
